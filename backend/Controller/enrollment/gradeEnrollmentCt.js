...
exports.updateGradeEnrollment = async (req, res) => {
  ...
  const session = await GradeEnrollment.db.startSession();
  session.startTransaction();
  try {
    // Check for duplicate documents with same filter
    const duplicates = await GradeEnrollment.find(filter).session(session);
    ...
    await GradeEnrollment.updateOne(filter, update, { upsert: true, session });
...
exports.deleteGradeEnrollment = async (req, res) => {
  ...
  try {
    ...
    // Unset gradeId for these members
    await MembersData.updateMany(
      { _id: { $in: ids } },
      { $unset: { gradeId: "" } }
    );
    if (result.modifiedCount > 0) {
      res.status(200).json({ message: `Member(s) removed from ${arrayField} array and gradeId unset`, removed: ids });
    } else {
