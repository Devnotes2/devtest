...
exports.deleteGradeBatchSubjectEnrollment = async (req, res) => {
  ...
  try {
    ...
    // Unset gradeBatchSubjectId for these members
    await MembersData.updateMany(
      { _id: { $in: ids } },
      { $unset: { gradeBatchSubjectId: "" } }
    );
    if (result.modifiedCount > 0) {
      res.status(200).json({ message: `Member(s) removed from ${arrayField} array and gradeBatchSubjectId unset`, removed: ids });
    } else {
