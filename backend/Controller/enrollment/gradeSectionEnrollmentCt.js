...
exports.getGradeSectionEnrollments = async (req, res) => {
  ...
  try {
    if (id) {
      const enrollment = await handleCRUD(GradeSectionEnrollment, 'findOne', { _id: new ObjectId(id) });
      if (!enrollment) return res.status(404).json({ message: 'Enrollment not found' });
      return res.json(enrollment);
    } else {
      const enrollments = await handleCRUD(GradeSectionEnrollment, 'find', {});
...
exports.deleteGradeSectionEnrollment = async (req, res) => {
  ...
  try {
    const result = await handleCRUD(GradeSectionEnrollment, 'delete', { _id: { $in: ids } });
