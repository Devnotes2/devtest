...
exports.updateGradeSectionEnrollment = async (req, res) => {
  ...
  const session = await GradeSectionEnrollment.db.startSession();
  session.startTransaction();
  try {
    // Check for duplicate documents with same filter
    const duplicates = await GradeSectionEnrollment.find(filter).session(session);
    ...
    await GradeSectionEnrollment.updateOne(filter, update, { upsert: true, session });
...
exports.deleteGradeSectionEnrollment = async (req, res) => {
  ...
  try {
    ...
    // Unset gradeSectionId for these members
    await MembersData.updateMany(
      { _id: { $in: ids } },
      { $unset: { gradeSectionId: "" } }
    );
    if (result.modifiedCount > 0) {
      res.status(200).json({ message: `Member(s) removed from ${arrayField} array and gradeSectionId unset`, removed: ids });
    } else {
