...
exports.getGradeSubjectEnrollments = async (req, res) => {
  ...
  const { ObjectId } = require('mongoose').Types;
  const { instituteId, academicYearId, gradeId, gradeSubjectId } = req.query;
  try {
    ...
    if (req.query.memberInfo === 'true' && req.query.memberType) {
      ...
      const enrollments = await GradeSubjectEnrollment.find(match);
      let memberIds = [];
    ...
    const enrollments = await GradeSubjectEnrollment.aggregate(pipeline);
...
exports.updateGradeSubjectEnrollment = async (req, res) => {
  ...
  if (!instituteId || !academicYearId || !gradeId || !gradeSubjectId || !memberType || !Array.isArray(updatedData)) {
  ...
  const filter = { instituteId, academicYearId, gradeId, gradeSubjectId };
  let arrayField;
  ...
  const session = await GradeSubjectEnrollment.db.startSession();
  session.startTransaction();
  try {
    // Check for duplicate documents with same filter
    const duplicates = await GradeSubjectEnrollment.find(filter).session(session);
    ...
    await GradeSubjectEnrollment.updateOne(filter, update, { upsert: true, session });

    // Find the upserted document (either existing or newly created)
    const upsertedDoc = await GradeSubjectEnrollment.findOne(filter).session(session);
...
exports.validateGradeSubjectEnrollment = async (req, res) => {
  const GradeSubjectEnrollment = createGradeSubjectEnrollmentModel(req.collegeDB);
  const MembersData = require('../../Model/membersModule/memberDataMd')(req.collegeDB);
  const { instituteId, academicYearId, gradeId, gradeSubjectId, memberType } = req.query;
  ...
  try {
    ...
    const enrollmentDoc = await GradeSubjectEnrollment.findOne(filter);
    // Find members by memberId
    const members = await MembersData.find({ memberId: { $in: ids } }, { _id: 1, memberId: 1, fullName: 1, gradeSubjectId: 1 });
    ...
    let response = ids.map(memberId => {
      ...
      // Check if enrolled under current gradeSubject
      ...
      // Check if member is enrolled under any gradeSubject
      if (Array.isArray(member.gradeSubjectId)) {
        if (member.gradeSubjectId.map(x => x.toString()).includes(gradeSubjectId)) {
          invalidCounter++;
          return { _id: member._id, memberId: member.memberId, fullName: member.fullName, description: 'Already enrolled' ,isValid:false};
        } else if (member.gradeSubjectId.length > 0) {
          return { _id: member._id, memberId: member.memberId, fullName: member.fullName, description: `valid`,isValid:true };
        }
      } else if (member.gradeSubjectId && member.gradeSubjectId.toString() === gradeSubjectId) {
        invalidCounter++;
        return { _id: member._id, memberId: member.memberId, fullName: member.fullName, description: 'Already enrolled', isValid:false };
      } else if (member.gradeSubjectId) {
        return { _id: member._id, memberId: member.memberId, fullName: member.fullName, description: `valid`,isValid:true };
      }
...
exports.deleteGradeSubjectEnrollment = async (req, res) => {
  ...
  try {
    ...
    const result = await GradeSubjectEnrollment.updateOne(filter, update);
    // Unset gradeSubjectId for these members
    await MembersData.updateMany(
      { _id: { $in: ids } },
      { $unset: { gradeSubjectId: "" } }
    );
    if (result.modifiedCount > 0) {
      res.status(200).json({ message: `Member(s) removed from ${arrayField} array and gradeSubjectId unset`, removed: ids });
    } else {
